#!/usr/bin/env python3
"""One-shot: listen for UG65 MQTT messages and insert into ug65 table."""
...
def on_message(client, userdata, msg):
    ...
    count += 1
    print(f"[ug65] id={row_id} topic={msg.topic} dev_eui={record.get('dev_eui')}")
    print(json.dumps(record.get("payload_json"), ensure_ascii=False))


client = mqtt.Client(
    mqtt.CallbackAPIVersion.VERSION2,
    client_id="ug65-catchup-once",
...
print(f"Listening {settings.mqtt_ug65_topic} for 30s...")
client.loop_start()
time.sleep(30)
client.loop_stop()
client.disconnect()
